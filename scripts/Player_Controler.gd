...
var heafbob_amplitude = 0.5#0.04
...
func headbob(time) -> Vector3:
	...
	pos.y = sin(time * headbob_frequency) * heafbob_amplitude
	#Side to side headbob: freq / 2 so that every second "step" is on the left, alternating from l to r
	pos.x = cos(time * headbob_frequency / 2) * heafbob_amplitude * headbob_side_to_side_strength
	return pos
